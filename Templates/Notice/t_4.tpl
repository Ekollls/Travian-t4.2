...
while($row = mysql_fetch_array($sql2)){ 
    $id = $row["id"];
    $toWref = $row["toWref"];
    $topics = explode('[=]',$row["topic"]); $topCount = count($topics);// var_dump($topCount); die;
	if($topCount==2) {
...
<input type="hidden" name="t" id="t" value="<?php echo $_GET['t']; ?>">
	<table cellpadding="1" cellspacing="1" id="overview" class="row_table_data">
		<thead><tr><th colspan="2"><?php echo SUBJECT;?>:</th><th class="sent"><?php echo SENT;?></th></tr></thead>
        <tbody>
   <?php 
...
<?php if($session->plus) { ?>
<button name="start" type="submit" value="start" id="start" class="green delete">
	<div class="button-container addHoverClick ">
		<div class="button-background">
			<div class="buttonStart">
				<div class="buttonEnd">
