...
if(isset($_GET['aid']) && $_GET['aid']==$session->alliance){
	...
	if($topCount==2) {if($topics[0]=='REPORT_REINFATTACKED'){$showAllTroops=false;} $t1 = explode('[|]',$topics[1]); if(defined($t1[0]))$t1[0]=constant($t1[0]); for($i=1;$i<count($t1);$i++) $t1[0].=$t1[$i];
		$topic = sprintf(constant($topics[0]),$t1[0]);} elseif($topCount==3) {$t1 = explode('[|]',$topics[1]); if(defined($t1[0]))$t1[0]=constant($t1[0]); for($i=1;$i<count($t1);$i++) $t1[0].=$t1[$i];
...
if (($noticeOwnerUid != $defenderUid)&&($noticeOwnerUid != $attackerUid)){ $showThemAll = false;} else {$showThemAll = true;}
if($session->access>=MULTIHUNTER) {$showThemAll = true;$showAllTroops=true;}
		...
		<tr>
			<th colspan="2">
				...
				<div id="time">
					...
					<div class="toolList">
						<?php if($session->plus){ ?>
							<button type="button" value="reportButton delete" class="icon" title="<?php echo REPORT_DEL_BTN; ?>"
	...
	<tbody>
		<tr>
			<td colspan="2" class="report_content">
				...
				<?php if ($showThemAll && $showAllTroops) { ?>
				<table id="attacker" cellpadding="0" cellspacing="0">
					<thead>
						<tr>
							...
							<td class="troopHeadline" colspan="11">
								<a href="spieler.php?uid=<?php echo $database->getUserField($dataarray[0],"id",0); ?>"><?php echo $database->getUserField($dataarray[0],"username",0); ?></a> <?php echo REPORT_FROM_VIL; ?> <a href="karte.php?d=<?php echo $dataarray[1]."&amp;c=".$generator->getMapCheck($dataarray[1]); ?>"><?php echo $da1name; ?></a>
								<div class="toolList">
									<?php if($session->plus){ ?>
					...
					<tbody class="units">
						...
						if ($dataarray[166]>0) { ?>
							<tr>
								<th><?php echo '<img class="unit u199" src="img/x.gif"/> '.REPORT_TRAPPED; ?></th>
								<?php
								for($i=167;$i<=176;$i++) {
									if($dataarray[$i] == 0) {
						...
						<tbody class="goods">
							<tr>
								...
								<td colspan="11">
									...
									<img class="unit g<?php echo $dataarray[152]; ?>Icon" src="img/x.gif" alt="<?php echo constant('B'.$dataarray[152]); ?>" title="<?php echo constant('B'.$dataarray[152]); ?>" />
									<?php	
										$ramp = explode('[=]',$dataarray[153]);
										if(count($ramp)==2){echo sprintf(constant($ramp[0]),$ramp[1]);}
										else { echo constant($ramp[0]);}
									?>
								</td>
							</tr>
						</tbody>
						...
						<tbody class="goods">
							<tr>
								...
								<td colspan="11">
									...
									<img class="unit g<?php echo $dataarray[155]; ?>Icon" src="img/x.gif" alt="<?php echo constant('B'.$dataarray[155]); ?>" title="<?php echo constant('B'.$dataarray[155]); ?>" />
									<?php	
					...
					<tbody class="goods">
						<tr>
							<th><?php echo REPORT_INFORMATION; ?></th>
							<td colspan="11">
								<img class="unit u<?php echo $dataarray[157]; ?>" src="img/x.gif" alt="<?php echo $technology->unarray[$dataarray[157]]; ?>" title="<?php echo $technology->unarray[$dataarray[157]]; ?>" />
								<img class="unit g<?php echo $dataarray[158]; ?>Icon" src="img/x.gif" alt="<?php echo constant('B'.$dataarray[158]); ?>" title="<?php echo constant('B'.$dataarray[158]); ?>" />
								<?php	
					...
					<tbody class="goods">
						<tr>
							<th><?php echo REPORT_INFORMATION; ?></th>
							<td colspan="11">
								<img class="unit u<?php echo $dataarray[160]; ?>" src="img/x.gif" alt="<?php echo $technology->unarray[$dataarray[160]]; ?>" title="<?php echo $technology->unarray[$dataarray[160]]; ?>" />
								<?php
									$lps = explode('[=]',$dataarray[161]);
									if(count($lps)==3) {echo sprintf(constant($lps[0]),$lps[1],$lps[2]);}
									elseif(count($lps)==1) {echo constant($lps[0]);}
								?>
							</td>
						</tr>
					</tbody>
						...
						<tr>
							...
							<td colspan="11">
								...
								<?php 
									...
									if(count($herop)==1){echo (defined($dataarray[165])?constant($dataarray[165]):$dataarray[165]);}
									elseif(count($herop)==2){echo sprintf(constant($herop[0]),$herop[1]);}
								?>
							</td>
						</tr>
					</tbody>
					...
					<tbody class="goods">
						<tr>
							...
							<td colspan="11">
								<div class="res">
									<div class="rArea"><img class="r1" src="img/x.gif" title="<?php echo LUMBER; ?>"><?php echo $dataarray[25]; ?></div>
									<div class="rArea"><img class="r2" src="img/x.gif" title="<?php echo CLAY; ?>"><?php echo $dataarray[26]; ?></div>
									<div class="rArea"><img class="r3" src="img/x.gif" title="<?php echo IRON; ?>"><?php echo $dataarray[27]; ?></div>
									<div class="rArea"><img class="r4" src="img/x.gif" title="<?php echo CROP; ?>"><?php echo $dataarray[28]; ?></div>
								</div>
								<div class="clear"></div>
								<div class="carry">
									<?php
