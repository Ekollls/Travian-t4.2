...
<div class="contentNavi tabNavi">
                ...
                <div class="container active">
					<div class="background-start">&nbsp;</div>
					<div class="background-end">&nbsp;</div>
					<div class="content"><a href="build.php?id=39&amp;t=9"><span class="tabItem">گریز از حمله</span></a></div>
				</div>
				<?php } ?>
</div>
<?php
if(isset($_GET['disable'])){
$database->query("UPDATE `s1_users` SET `escape`=0 WHERE `id` = '".$session->uid."'");		header("Location: build.php?id=39&t=9");		echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=9';</SCRIPT>";
}
if(isset($_GET['enable'])){
$database->query("UPDATE `s1_users` SET `escape`=1 WHERE `id` = '".$session->uid."'");		header("Location: build.php?id=39&t=9");		echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=9';</SCRIPT>";
}
...
<br><br><br>
		<input type="checkbox" class="check" name="hideShow" onclick="window.location.href = '?id=39&t=9&<?php if($evasion['escape'] == 1){ echo "disable"; } else{ echo "enable"; } ?>';"  <?php if($evasion['escape']==1){ echo "checked=\"checked\""; }else{ echo ""; } ?>> 
